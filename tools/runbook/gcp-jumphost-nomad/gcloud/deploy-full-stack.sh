#!/usr/bin/env bash
# deploy-full-stack.sh — Deploy complete platform stack ke GCP Nomad VM
#
# Layer layout (semua di 1 server untuk sekarang):
#   controller  → Nomad, Consul, platform-api
#   worker      → Firecracker agents, FC pool
#   data        → MinIO, PostgreSQL, Redis, Jaeger
#
# Konfigurasi host per layer dibaca dari config/topology.env.
# Buat file tersebut dengan: ./gcloud/gen-topology.sh
#
# Usage:
#   ./deploy-full-stack.sh [OPTIONS]
#
# Options:
#   --skip-sync      Skip rsync services/
#   --skip-firewall  Skip gcloud firewall rule
#   --skip-nomad     Skip platform-api Nomad redeploy
#   --my-ip IP       Public IP Anda (default: auto-detect)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNBOOK_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
ROOT_DIR="$(cd "${RUNBOOK_DIR}/../../../.." && pwd)"
...
if [[ -f "${TOPOLOGY_FILE}" ]]; then
  ...
  echo "WARN: config/topology.env tidak ditemukan." >&2
  echo "  Jalankan:  ./gcloud/gen-topology.sh" >&2
  echo "  Atau copy: cp config/topology.env.example config/topology.env  lalu isi IP." >&2
  echo ""
fi

# ── Config (fallback ke env vars jika topology tidak ada) ────────────────────
...
# Layer hosts — semua default ke CONTROLLER_HOST jika tidak di-set terpisah
...
# ── Step 1: Sync services/ (data layer config) ────────────────────────────────
if [[ "${SKIP_SYNC}" == "false" ]]; then
  echo "[1/4] Syncing services/ ke VM (data layer config)..."
...
# ── Step 2: Start data layer containers ──────────────────────────────────────
echo "[2/4] Starting data layer containers (MinIO, PG, Redis, Consul, Jaeger)..."
...
if [[ "${SKIP_FIREWALL}" == "false" ]]; then
  echo "[3/4] Opening firewall ports for ${MY_CIDR}..."

  RULE_NAME="platform-dev-access"
  # controller ports: 4646 (Nomad), 8500 (Consul), 8080 (platform-api)
  # data ports: 9001 (MinIO console), 16686 (Jaeger), 4317/4318 (OTEL)
...
# ── Step 4: Deploy controller layer (platform-api Nomad job) ─────────────────
if [[ "${SKIP_NOMAD}" == "false" ]]; then
  echo "[4/4] Deploying controller layer: platform-api Nomad job..."
...
# data layer: karena semua di 1 server, address = 127.0.0.1
# Saat pindah ke server terpisah, ganti ke IP data layer.
DATA_HOST="127.0.0.1"
...
EOF
  ...
  echo ""
  echo "  platform-api (controller layer) deployed."
else
